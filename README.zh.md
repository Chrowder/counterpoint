...
> **🔗 关于在线 demo:** 部署的网站只是**一次 NVDA 研究的静态快照**——在浏览器里回放那一轮录制好的流程,不连后端、不跑真实 agent、不调用任何 API,仅供展示,并非可用的投研台。想对任意股票做**真实研究**,需要克隆本仓库并配置自己的 API 密钥(Anthropic + 第二个模型家族 + Finnhub + Band),见下方「环境准备」与「运行」。
...
| M8 复盘 | Chair 用本轮真实数据逐条检查上次的改判条件(kill criteria)兑现没,备忘录加「往期对比与复盘」,把两次分析连成可检验判断链 |
