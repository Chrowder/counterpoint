...
- 中英双语是**配置轴不是分支**:`OUTPUT_LANG=en|zh`(**本分支默认 en**,i18n.DEFAULT="en")决定备忘录/辩论/证据包/工具描述的语言,换语言改 env、不改代码。产出层走 `counterpoint/i18n.py`(`output_lang()`/`pick({"zh","en"})`),各 agent prompt / chair 工具描述与节标题 / evidence 字段标签 / memory 脚手架都按它切;缺该语言回落 DEFAULT(en)。跨运行记忆按 lang 隔离(savememo 记 `lang`,recall 只回放同语言,旧记录无 lang 视为 zh)。Web UI 语言**独立**于 OUTPUT_LANG,默认 en,走 `web/ui/src/i18n.js`(`?lang`/localStorage + 右上角切换),阶段标题前端按 stage.key 映射。**测试钉在 zh**(`tests/conftest.py` 设 OUTPUT_LANG=zh,因多数单测断言中文产出、且 chair import 时即解析 T)。完成后房间快照落 `data/rooms/<T>.json`(gitignored):清场前定格,`/api/room`+`/api/progress` 在房间 teardown 后改读它,免得进度跌回 2/6、直播消失。文档默认英文(`README.md`/`web/README.md`),中文版 `README.zh.md`/`web/README.zh.md`,顶部互挂。一次只能跑一种语言的台。
