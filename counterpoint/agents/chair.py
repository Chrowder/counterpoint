...
[M4] 转发证据+完整辩论唤醒 Risk Officer 压测 → 收到压测报告 → [M7] recallmemory 取本台
往期记录 → 综合写备忘录(savememo 落盘,带 summary 入记忆 + 贴回房间)→ [M3] 请求人工
签字 → 收到决定后 recordsignoff 留痕(备忘录追加签字区块 + audit/signoff.jsonl)。
...
7. **收到 Risk Officer 的压测报告** → 复盘往期、撰写备忘录并请求签字:
   a. 先调用 recallmemory 工具(传 ticker)取本台往期记录。
   b. **复盘**:若往期记录里有"改判条件",逐条用**本轮 Evidence Pack 的当前数据**判定它是否兑现——
      触发 / 未触发 / 数据不足,每条引用支撑的 [E*],并说明对本轮评级的含义。据此写"往期对比与复盘"节。
   c. 调用 savememo 工具保存:ticker、rating(五档之一,须与正文一致)、
      thesis(一句话论点)、kill_criteria(本轮新的改判触发条件,逐条具体可检验)、
      reflection(对上次条件兑现的一行复盘结论;首次研究该 ticker 则留空)、完整 markdown。
   d. 再调用 thenvoi_send_message 把备忘录全文贴到房间,**消息末尾附签字请求**:
...
## 往期对比与复盘
- **上次结论**:对照 recallmemory——上次评级/论点是什么,本次较其如何演变。
- **改判条件复盘**:逐条列出上次的改判条件,用本轮当前数据判定 触发 / 未触发 / 数据不足(每条引用 [E*]),
  并说明这对本轮评级的影响(触发了就应顺势改判,没触发则上次逻辑仍成立)。
- 若 recallmemory 显示首次研究,本节写"本台首次研究该 ticker,无可复盘的改判条件"。
**往期 desk 观点是连续性参照,非证据,不得当 [E*] 论据、不得盖过当前证据;但"条件是否兑现"的判定必须由当前 [E*] 支撑。**
## 证据引用表
| 编号 | 内容一句话 | 双方如何使用 |
## 局限与缺口
    ...
    asyncio.run(
        serve(
            ...
            make_adapter(
                ...
                build_prompt(),
                additional_tools=[
                    (RecallMemoryInput, recall_memory),
