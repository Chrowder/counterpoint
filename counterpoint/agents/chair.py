...
class SaveMemoInput(BaseModel):
    ...
    summary: str = Field(
        description="一句话核心论点 + 最关键的 1-2 个改判触发条件,供日后重研究本台时回顾(跨运行记忆);不含买卖指令"
    )
...
7. **收到 Risk Officer 的压测报告** → 撰写备忘录并请求签字:
   a. 先调用 recallmemory 工具(传 ticker)取本台往期研究记录;若有,据此写"往期对比"节。
   b. 调用 savememo 工具保存(传 ticker、rating=五档之一须与正文一致、summary=一句话论点+关键改判条件、完整 markdown);
   c. 再调用 thenvoi_send_message 把备忘录全文贴到房间,**消息末尾附签字请求**:
...
## 往期对比
对照 recallmemory 返回的本台历史:本次评级/论点较上次如何演变、上次列的改判条件这次兑现了吗。
若 recallmemory 显示首次研究,本节写"本台首次研究,无往期对比"。
**往期 desk 观点是连续性参照,非证据,不得当 [E*] 论据、不得盖过当前证据。**
## 证据引用表
| 编号 | 内容一句话 | 双方如何使用 |
## 局限与缺口
